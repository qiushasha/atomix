 ...
 */
public abstract class AbstractConsistentMapProxy<S extends ConsistentMapService>
    extends AbstractAsyncPrimitiveProxy<AsyncConsistentMap<String, byte[]>, S>
  ...
  public synchronized CompletableFuture<Void> addListener(MapEventListener<String, byte[]> listener, Executor executor) {
    if (mapEventListeners.isEmpty()) {
      ...
      return acceptAll(service -> service.addListener()).thenApply(v -> null);
    } else {
      mapEventListeners.put(listener, executor);
      return CompletableFuture.completedFuture(null);
    }
  ...
  public synchronized CompletableFuture<Void> removeListener(MapEventListener<String, byte[]> listener) {
    if (mapEventListeners.remove(listener) != null && mapEventListeners.isEmpty()) {
      return acceptAll(service -> service.removeListener()).thenApply(v -> null);
    }
    return CompletableFuture.completedFuture(null);
  }
        ...
        .thenRun(() -> getPartitionIds().forEach(partition -> {
          addStateChangeListenerOn(partition, state -> {
            if (state == PartitionProxy.State.CONNECTED && isListening()) {
              acceptOn(partition, service -> service.addListener());
            }
          });
        }))
        .thenApply(v -> this);
